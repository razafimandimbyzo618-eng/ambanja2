    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 no-print">
        ...
        <div className="bg-emerald-600 p-5 rounded-3xl shadow-lg shadow-emerald-100/50 flex flex-col justify-between relative overflow-hidden group">
            <div className="absolute -right-4 -top-4 w-24 h-24 bg-white/10 rounded-full blur-2xl group-hover:scale-150 transition-transform duration-700" />
            <div className="relative">
                <p className="text-[16px] font-bold text-emerald-100 uppercase tracking-widest mb-1">Total des Avances</p>
                <p className="text-[14px] font-medium text-emerald-200 uppercase tracking-[0.2em]">Aujourd'hui</p>
            </div>
        ...
        <div className="bg-white/70 backdrop-blur-md p-5 rounded-3xl border border-emerald-50/50 flex items-center justify-between shadow-sm">
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <input 
                        type="date" 
                        className="bg-transparent border-none outline-none font-bold text-emerald-900 text-base p-0 h-auto cursor-pointer"
                        value={dateFilter}
      ...
      <div className="flex flex-col xl:flex-row justify-between items-stretch xl:items-center bg-white/60 backdrop-blur-md p-3 rounded-2xl border border-emerald-50/50 gap-4 shadow-sm no-print">
          ...
          <div className="flex bg-emerald-50/30 p-1 rounded-xl border border-emerald-100/50">
            {['all', 'day', 'month'].map(f => (
              <button 
                ...
                className={`px-3 py-1.5 rounded-lg text-[15px] font-bold uppercase tracking-wider transition-all ${frequencyFilter === f ? 'bg-emerald-600 text-white shadow-sm' : 'text-emerald-600/70 hover:bg-emerald-50'}`}
              >
        ...
        <div className="flex items-center gap-3">
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2 bg-white border border-emerald-100 text-emerald-600 rounded-xl text-[16px] font-bold uppercase hover:bg-emerald-50 transition-all shadow-sm"
          >
      ...
      <div className="min-h-[400px]">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 animate-pulse">
            ...
            <p className="text-[16px] font-bold text-emerald-800 uppercase tracking-widest">Chargement...</p>
          </div>
        ) : filtered.length === 0 ? (
            ...
            {groupedList.map((invoice) => (
              <div key={invoice.facture_id} className="bg-white/80 border border-emerald-50/50 rounded-3xl overflow-hidden shadow-sm hover:shadow-md hover:-translate-y-1 transition-all group">
                <div className="p-5 bg-emerald-50/20 border-b border-emerald-50 flex justify-between items-center">
                  <div>
                    <h4 className="font-bold text-emerald-900 text-xl leading-none mb-1">{invoice.number}</h4>
                    <p className="text-[15px] font-bold text-emerald-600/80 uppercase tracking-widest">{invoice.client_name}</p>
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xl font-bold text-emerald-700">{invoice.total_remaining.toLocaleString()} Ar</p>
                  </div>
                </div>
                  ...
                  {invoice.installments.map((inst, idx) => {
                    ...
                    return (
                      <div key={inst.id} className="flex items-center justify-between p-3.5 rounded-2xl border border-emerald-50/30 bg-white/50 group/item hover:bg-emerald-50/30 transition-all">
                        <div className="flex items-center gap-3">
                          <div className={`w-8 h-8 rounded-lg flex items-center justify-center font-bold text-base ${isOverdue ? 'bg-red-50 text-red-500 border border-red-100' : 'bg-emerald-50 text-emerald-600'}`}>
                            {idx + 1}
                          </div>
                          <div>
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <button 
                            ...
                            className="bg-emerald-600 text-white px-3 py-1.5 rounded-lg shadow shadow-emerald-100 hover:bg-emerald-700 transition-all flex items-center gap-1.5 text-[15px] font-bold uppercase tracking-wider"
                          >
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-emerald-50/20 border-b border-emerald-50/50">
                  ...
                  <th className="p-4 text-[15px] font-bold text-emerald-700 uppercase tracking-widest">Échéance</th>
                  <th className="p-4 text-[15px] font-bold text-emerald-700 uppercase tracking-widest text-right">Global</th>
                  <th className="p-4 text-[15px] font-bold text-emerald-700 uppercase tracking-widest text-right">Avances</th>
                  <th className="p-4 text-[15px] font-bold text-emerald-700 uppercase tracking-widest text-right">Dû</th>
                  <th className="p-4 text-[15px] font-bold text-emerald-700 uppercase tracking-widest text-center">Action</th>
                </tr>
              </thead>
                ...
                {filtered.map(d => {
                  ...
                  return (
                    <tr key={d.id} className="hover:bg-emerald-50/10 transition-colors">
                      ...
                      <td className="p-4">
                        <span className={`text-[16px] font-bold uppercase px-2 py-1 rounded-md ${isOverdue ? 'bg-red-50 text-red-500 border border-red-100/50' : 'bg-emerald-50 text-emerald-600 border border-emerald-100/50'}`}>
                      ...
                      <td className="p-4 font-bold text-emerald-600 text-right text-[17px]">{d.totalPaid.toLocaleString()} Ar</td>
                      ...
                      <td className="p-4 text-center">
                        <div className="flex items-center justify-center gap-1.5">
                            ...
                            <button 
                                ...
                                className="bg-emerald-600 text-white px-3 py-1.5 rounded-lg text-[15px] font-bold uppercase tracking-widest hover:bg-emerald-700 transition-all flex items-center gap-1.5 shadow shadow-emerald-100"
                            >
                                <Banknote size={12} /> Payer
                            </button>
                        </div>
          ...
          <div className="bg-white/70 backdrop-blur-md border border-emerald-50/50 rounded-3xl p-6 shadow-sm">
            <div className="flex items-center justify-between mb-8">
              <h3 className="text-2xl font-bold text-emerald-900 uppercase tracking-widest flex items-center gap-3">
                  ...
                  <div key={day} className={`aspect-square border rounded-2xl p-2 flex flex-col items-center justify-between transition-all ${dayDeadlines.length > 0 ? 'border-emerald-200 bg-emerald-50/30 shadow-sm shadow-emerald-100/20' : 'border-gray-50'}`}>
                    ...
                    {dayDeadlines.length > 0 && (
                      <div className="flex flex-col gap-1 w-full mt-1">
                        {dayDeadlines.map(d => (
                          <div key={d.id} onClick={() => setPaymentModal({ echeance: d })} className="bg-emerald-600 text-white text-[13px] font-bold p-1 rounded-md truncate cursor-pointer hover:bg-emerald-700 shadow-sm transition-colors">
                            {d.factures?.number}
                          </div>
                        ))}
                      </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-[320px] overflow-hidden animate-in zoom-in-95 duration-200 border border-gray-100">
            ...
            <div className="p-5 space-y-4">
              <div className="bg-gray-50/50 p-3.5 rounded-xl border border-gray-100/50">
                ...
                <p className="text-3xl font-bold text-emerald-700">{paymentModal.echeance.montant.toLocaleString()} Ar</p>
              ...
              <div className="space-y-3">
                <div className="space-y-1">
                  ...
                  <input 
                    type="number" 
                    className="w-full bg-gray-50/50 border border-gray-100 rounded-lg px-3 py-2 text-base outline-none focus:border-emerald-500 transition-all font-bold text-emerald-900"
                    value={paymentAmount || ''}
                ...
                <div className="space-y-3">
                    ...
                    <div className={`grid grid-cols-4 gap-1 p-1 rounded-lg transition-all ${!paymentInfo.method ? 'bg-red-50 border border-red-100' : 'bg-gray-50/50 border border-gray-100'}`}>
                      {['esp', 'mm', 'vir', 'cheq'].map(m => (
                        <button 
                          ...
                          className={`py-1.5 rounded-md text-[13px] font-bold uppercase tracking-widest border transition-all ${paymentInfo.method === m ? 'bg-emerald-600 border-emerald-600 text-white shadow-sm' : 'bg-white/80 border-gray-100 text-gray-400 hover:text-emerald-600'}`}
                        >
                          {m}
                        </button>
                      ))}
                  ...
                  <div className="space-y-1">
                    ...
                    <input 
                      type="text" 
                      className="w-full bg-gray-50/50 border border-gray-100 rounded-lg px-3 py-2 text-base outline-none focus:border-emerald-500 transition-all font-medium text-gray-600"
              ...
              <button 
                ...
                className="w-full bg-emerald-600 text-white font-bold py-3 rounded-xl shadow-lg shadow-emerald-100/50 active:scale-95 transition-all flex items-center justify-center gap-2 text-[16px] uppercase tracking-widest disabled:bg-gray-200 mt-2"
              >
          ...
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-200 border border-emerald-50">
            <div className="p-6 border-b border-emerald-50 bg-emerald-50/20 flex justify-between items-center">
              <div>
                ...
                <p className="text-[15px] font-medium text-emerald-600/80 uppercase mt-1">{selectedInvoiceHistory.number}</p>
              </div>
              <button 
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-emerald-50/30 p-3 rounded-2xl border border-emerald-100/50">
                  ...
                  <p className="text-base font-bold text-emerald-900">{Number(selectedInvoiceHistory.total_amount).toLocaleString()} Ar</p>
                </div>
